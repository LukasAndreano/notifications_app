  ...
  return (
    <Group>
      <Placeholder
        ...
        header="Подключение уведомлений от сервиса КИТЭК"
      >
        Сервис уведомляет пользователя при обновлении расписания занятий.
      </Placeholder>
      <FormLayout
        onSubmit={(e) => {
          e.preventDefault();
          setDisabled(true);
          if (!disabled) {
            api("connectService", "id=6").then((data) => {
              setDisabled(false);
              if (data.response) {
                dispatch(
                  setSnackbar({
                    text: "Уведомления от «КИТЭК» включены!",
                    success: true,
                  })
                );
